namespace FarmTracker
{
    ...
    public sealed partial class FarmTrackerCore
    {
        ...
        private void HandleZoneTransition(AreaInstance area, WorldAreaDat details, string inst)
        {
            ...
            var treatSubAreas = this.Settings.TreatSubAreasAsSameMap;
            var isSubArea = treatSubAreas && FarmMapSubAreaRules.IsSubArea(details.Id);

            if (isMap)
            {
                if (this.TryContinueSubAreaRun(details, inst, isSubArea, now))
                {
                    return;
                }
        ...
        private bool TryContinueSubAreaRun(WorldAreaDat details, string inst, bool isSubArea, DateTime now)
        {
            if (!this.Settings.TreatSubAreasAsSameMap || this.currentMap == null)
            {
                return false;
            }
            ...
            {
                this.onMapArea = true;
                this.inMapSubArea = true;
                this.mapRunStartUtc ??= now;
                return true;
            }

            if (this.inMapSubArea && inst.Equals(this.currentMap.Hash, StringComparison.Ordinal))
            {
                this.onMapArea = true;
                this.inMapSubArea = false;
                this.mapRunStartUtc ??= now;
