namespace FarmTracker
{
    ...
    public sealed partial class FarmTrackerCore
    {
        ...
        private void DrawOverlaySettingsTab()
        {
            ...
            ImGui.Checkbox(
                L("Sub-areas count as same map (Abyss, Breach, …)", "Sub-Zonen zaehlen zur gleichen Map (Abyss, Breach, …)"),
                ref this.Settings.TreatSubAreasAsSameMap);
            ImGuiHelper.ToolTip(L(
                "Abyssal Depths, Breach domains, expedition side areas, etc. keep the same map timer and loot run instead of starting a new map.",
                "Abyssal Depths, Breach-Domains, Expedition-Nebenzonen usw. behalten denselben Map-Timer und Loot-Lauf."));
