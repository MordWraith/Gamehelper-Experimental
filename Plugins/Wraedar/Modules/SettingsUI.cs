...
public sealed class SettingsUI : PluginModule {
    ...
    //--| Draw |-------------------------------------------------------------------------------------------------------
    public void Draw() {
        Settings.SettingsWindowOpen = false;
    ...
    private static readonly SVector2 conSize = new(120,22);
    ...
    private static float ToggleWidth(string label) => Math.Max(controlWidth, ImGui.CalcTextSize(label).X + 16f);

    private static void DrawToggle(string id, ref bool value, string label, DXT.Tooltip.Options? tooltip = null) {
        DXT.Button.DrawToggle(id, ref value, new DXT.Button.Options {
            Label = label,
            Width = ToggleWidth(label),
            Height = controlHeight,
            Tooltip = tooltip,
        });
    }

    private static void DrawIconName(string name) {
    ...
    public void RenderPinWindow() {
        if (!Settings.Pin.EditorWindowOpen) {
            return;
        }

        if (DXT.Window.Begin($"{DXT.PluginName}PinWindow", ref Settings.Pin.EditorWindowOpen, PinWindowOptions)) {
            ImGui.BeginChild("PinEditorScroll", SVector2.Zero, ImGuiChildFlags.None);
            ImGui.Indent(3);
    ...
    private void Draw_GeneralSettings(string panelID) {
        ...
        DrawToggle("DrawOverPoePanels", ref Settings.DrawOverPanels, "Over Panels", new("Draw over poe panels, only working for some panels missing game offsets for others"));
        ImGui.SameLine();
        DrawToggle("DrawIfBackground", ref Settings.DrawIfForegroundOnly, "Focused only", new("Draw when POE is focused only (foreground window)."));
        ImGui.SameLine();
        DrawToggle("DrawInSafeArea", ref Settings.DrawInSafeArea, "Safe Areas", new("Draw in towns and hideout"));
        ImGui.SameLine();
        DrawToggle("CenterOffset", ref Settings.DXTSettings.LargeMapCenterFix, "Map Fix", new("Fix LargeMap centering",
        ...
        ImGui.SameLine();
        DrawToggle("DebugToolbar", ref Settings.DXTSettings.ShowTools, "Debug", new("Show the Debug Toolbar/Tools"));

    }

    //--| Map Settings |---------------------------------------------------------------------------------------------------------------------------
    private void Render_MapSettings(string panelID) {
        DrawToggle("Map.Enabled", ref Settings.Map.Enabled, "Enabled", new("Draw Area Map (Maphack)"));
        ImGui.SameLine();
    ...
    private void Render_PinSettings(string panelID) {
        DrawToggle("Pin.Enabled", ref Settings.Pin.Enabled, "Enabled", new("Enable Pin Tracking, ie. Bosses, Area Transitions, quest locations"));
        ImGui.SameLine();
        DrawToggle("Pin.Paths", ref Settings.Pin.DrawPaths, "Draw Paths", new("Draw Paths, Draw Paths to Pins"));
        ImGui.SameLine();
        DrawToggle("Pin.OverrideColorPaths", ref Settings.Pin.OverrideColorPaths, "Color Cycle", new("Draw paths with a unique color per path, ignoring the colors set for each Pin. "));
        ...
        ImGui.SameLine();
        DrawToggle("Pin.EditorWindowOpen", ref Settings.Pin.EditorWindowOpen, "Pin Editor", new("Pin Editor", "Edit Pins"));
    ...
    private void Draw_IconSettings(string panelID) {
        DrawToggle("Icons.Enabled", ref Settings.Icons.Enabled, "Enabled", new("Draw Icons on map"));
        ImGui.SameLine();
        DrawToggle("Icons.PixelPerfect", ref Settings.Icons.PixelPerfect, "Pixel Perfect", new("Enables pixel-perfect icons for crisp, unblurred icons."));
        ImGui.SameLine();
        DrawToggle("Icons.DrawCached", ref Settings.Icons.DrawCached, "Draw Cached", new("Draw icons for entities you have already discovered, even if they are now outside your poe clients visible network range."));
    }
    ...
    private void Draw_IconsSettings() {
        ...
        foreach (var iconGroup in IconRegistry.Groups) {
            ...
            if (DXT.CollapsingPanel.Begin(iconGroup.Name, ref isCollapsed, new() { Label = iconGroup.Name, Width = 0 })) {
                ...
                ImGui.PopStyleVar();
                DXT.CollapsingPanel.End(iconGroup.Name);
            }
