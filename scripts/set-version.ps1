...
. (Join-Path $PSScriptRoot "component-versions.ps1")
...
function Set-ProjectVersion {
    ...
    $versionsPath = Get-VersionsFilePath -Root $Root
    if (Test-Path $versionsPath) {
        Set-ComponentVersion -Root $Root -Component core -Version $Version
        Set-ComponentVersion -Root $Root -Component launcher -Version $Version
        return
    }
...
function Write-BuildInfoFiles {
    ...
    if (Test-Path (Get-VersionsFilePath -Root $Root)) {
        ...
        $componentLines += "Core: $($summary.core)"
        $componentLines += "Launcher: $($summary.launcher)"
        $componentLines += "Downloader: $($summary.downloader)"
        foreach ($pluginId in ($summary.plugins.Keys | Sort-Object)) {
            $plugin = $summary.plugins[$pluginId]
            $kind = if ($plugin.core) { "core plugin" } else { "optional plugin" }
            $componentLines += "Plugin $pluginId ($kind): $($plugin.version)"
        }

        Write-ComponentVersionsFile -PublishDir $PublishDir -Root $Root
    }

  $versionTxt = @"
GameHelper core $Version
Gebaut am: $builtAt (UTC)
Quelle: $Source
$(if ($componentLines.Count -gt 0) { "`nKomponenten:`n$($componentLines -join "`n")`n" })
Pruefen: Rechtsklick auf GameHelper.App.exe -> Details -> Dateiversion (core)
Siehe auch: components.json
"@
...
Gebaut:  $builtAt (UTC)
$(if ($componentLines.Count -gt 0) { "`nKomponentenversionen: components.json im Installationsordner`n" })
