...
param(
    ...
    [string]$DownloaderRemoteName = "GameHelperDownloader.exe",
    [switch]$IncludeGithubConfigInPackage,
    [switch]$UseRootGithubConfig = $true,
    [ValidateSet("Debug", "Release")]
    [string]$Configuration = "Release",
    [switch]$SkipBuild,
    [switch]$SkipDownloader,
    [switch]$ExperimentalDownloader,
    [switch]$FullUpload,
...
if (-not $SkipBuild) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($PublishDirectory)) {
        $buildArgs.OutputDir = $PublishDirectory
    }
...
if ([string]::IsNullOrEmpty($Version)) {
    if (Test-Path (Get-VersionsFilePath -Root $Root)) {
        $Version = Get-PublishManifestVersion -Root $Root
    }
    else {
...
if (-not $IncludeGithubConfigInPackage) {
    $excludePatterns += "github.config.json"
}
...
if ($LegacyManifest) {
    ...
    if (Test-Path (Get-VersionsFilePath -Root $Root)) {
        ...
        $manifest["components"] = Build-ManifestComponentsBlock -Root $Root
    }
...
. (Join-Path $PSScriptRoot "set-version.ps1")
Write-BuildInfoFiles -PublishDir $PublishDir -Version $Version -Source "github-publish" -Root $Root
