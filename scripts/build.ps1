...
function Copy-DeployUserDataTree {
    param(
        [string]$SourceDir,
    ...
    foreach ($item in Get-ChildItem -Path $SourceDir -Force) {
        ...
        if ($item.PSIsContainer) {
            Copy-DeployUserDataTree -SourceDir $item.FullName -DestinationDir $destPath
        }
        else {
...
function Save-DeployUserData {
    ...
    if (Test-Path $configs) {
        Copy-DeployUserDataTree -SourceDir $configs -DestinationDir (Join-Path $BackupDir "configs")
    }
    ...
    foreach ($plugin in Get-ChildItem $pluginsRoot -Directory) {
        ...
        Copy-DeployUserDataTree -SourceDir $srcConfig -DestinationDir (Join-Path $BackupDir "Plugins\$($plugin.Name)\config")
    }
}
...
function Restore-DeployUserData {
    ...
    if (Test-Path $configs) {
        Copy-DeployUserDataTree -SourceDir $configs -DestinationDir (Join-Path $TargetDir "configs")
        $restored = $true
    }
    ...
    if (Test-Path $backupPlugins) {
        foreach ($plugin in Get-ChildItem $backupPlugins -Directory) {
            ...
            Copy-DeployUserDataTree -SourceDir $srcConfig -DestinationDir (Join-Path $TargetDir "Plugins\$($plugin.Name)\config")
            $restored = $true
        }
    }
...
function Test-IsPreservedDeployLauncher {
    param([string]$Name)

    return $Name -like 'rebuild*.bat' -or $Name -like 'rebuild*.ps1'
}
...
Bitte GameHelper schliessen und den Build erneut starten.
"@
    }
    ...
    foreach ($item in @(Get-ChildItem -Path $TargetDir -Force -ErrorAction SilentlyContinue)) {
        if (-not $item.PSIsContainer -and (Test-IsPreservedDeployLauncher $item.Name)) {
            continue
        }
    ...
    if ($failed.Count -gt 0) {
        $list = $failed -join ', '
        throw @"
Deploy-Inhalt in '$TargetDir' konnte nicht vollstaendig geleert werden ($list).
Bitte GameHelper schliessen, Explorer-Fenster in diesem Ordner schliessen, und Build erneut starten.
Details: Dateien sind noch von einem anderen Prozess gesperrt.
"@
    }
}
...
function Remove-DeployDirectory {
    ...
    Clear-DeployDirectory -TargetDir $TargetDir
}
...
try {
    ...
    if (Restore-DeployUserData -TargetDir $PublishDir -BackupDir $testUserBackup) {
        ...
        if (Test-Path $runtimeBackup) {
            if (Test-Path (Join-Path $runtimeBackup "configs")) {
                Copy-DeployUserDataTree -SourceDir (Join-Path $runtimeBackup "configs") -DestinationDir (Join-Path $PublishDir "configs")
                Repair-PluginsJson -JsonPath (Join-Path $PublishDir "configs\plugins.json")
            }
            foreach ($plugin in Get-ChildItem (Join-Path $PublishDir "Plugins") -Directory) {
                ...
                if (Test-Path $srcConfig) {
                    Copy-DeployUserDataTree -SourceDir $srcConfig -DestinationDir (Join-Path $plugin.FullName "config")
                }
            }
        }
    }
    ...
    if ($PublishDir -ne $defaultPublish) {
        if (-not (Test-Path $testUserBackup) -and (Test-Path $defaultPublish)) {
    ...
    Save-DeployUserData -SourceDir $PublishDir -BackupDir $testUserBackup
    Write-Host "  Test-Einstellungen in test-runtime-backup aktualisiert." -ForegroundColor DarkGray
    ...
    Write-BuildInfoFiles -PublishDir $PublishDir -Version $buildVersion -Source "local-build" -Root $Root
